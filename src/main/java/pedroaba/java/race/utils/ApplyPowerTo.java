...
public class ApplyPowerTo {
    public static void apply(Car targetToReceivePower, Car target) {
        ...
        switch (indexOfPower) {
            ...
            case 1:
                target.setPower(
                    new Boost(targetToReceivePower, random.nextDouble(Config.START_RANGE_OF_POWER_RANGE, Config.END_RANGE_OF_POWER_RANGE), Config.TIME_DURATION_OF_BOOST_POWER)
                );
                break;
            case 2:
                target.setPower(
